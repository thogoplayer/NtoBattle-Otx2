function onUse(cid, item, frompos, item2, topos)
    ...
    local function potion(pos, cid)
        ...
        doPlayerSendTextMessage(pos,22, 'Seu Golden Card Exp Acabou !!!')
    end
    ...
    if getPlayerStorageValue(cid, storage_cooldown) < os.time() then
        if item.itemid == 10684 then -- aqui esta o iD do item que você quer colocar pra usar
            setPlayerStorageValue(cid, storage_cooldown, timestamp_cooldown)
            doRemoveItem(item.uid, 1)
            doPlayerSetExperienceRate(cid, exp)
            doSendMagicEffect(frompos, 13)
            doPlayerSendTextMessage(cid, 22, 'Vocꠕtilizou o Golden Card Exp, estᠣom + 20% de Exp.')
