...
function onSay(cid, words, param, channel)
	...
	if(config.advancedFragList) then
		...
		result = result .. "\nFrags ganhos no mês: " .. (size.day + size.week + size.month) .. "."
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Você possui " .. size.day .. " frags no dia, " .. (size.day + size.week) .. " na semana e " .. (size.day + size.week + size.month) .. " no mês.")
		if(size.day > 0) then
