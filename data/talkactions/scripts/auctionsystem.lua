...
function onSay(cid, words, param, channel)
        ...
        if(t[1] == "add") then
                ...
                db.query("INSERT INTO `auction_system` (`player`, `item_name`, `item_id`, `count`, `cost`, `date`) VALUES (" .. getPlayerGUID(cid) .. ", \"" .. t[2] .. "\", " .. getItemIdByName(t[2]) .. ", " .. itemcount .. ", " .. costgp ..", " .. os.time() .. ")")
                                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You successfully add " .. itemcount .." " .. t[2] .." for " .. costgp .. " Points to offerts database.")
        end
        if(t[1] == "buy") then
                ...
                if(buy:getID() ~= -1) then
				local cost = buy:getDataInt("cost")
                        if(getPlayerPointsZNote(cid) < cost) then
                        ...
                        doPlayerRemovePointsZNote(cid, cost)
                        db.query("DELETE FROM `auction_system` WHERE `id` = " .. t[2] .. ";")
                        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You bought " .. buy:getDataInt("count") .. " ".. buy:getDataString("item_name") .. " for " .. cost.. " Points!")
        ...
        if(t[1] == "withdraw") then
                ...
                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You got " .. balance:getDataInt("auction_balance") .. " points from auction system!")
                doPlayerAddPointsZNote(cid, balance:getDataInt("auction_balance"))
